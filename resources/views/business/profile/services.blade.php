            ...
            @foreach($services as $service)
                <div id="service-{{ $service['id'] }}" class="service-card flex bg-[#1a1a1a] border border-[#374151] p-4 gap-4 hover:border-[#F3F4F6]/50 transition-all duration-300 group">
                    ...
                    <div class="flex-grow flex flex-col justify-between">
                        ...
                        <!-- Contenedor dinámico para evidencias -->
                        <div class="mt-3">
                            <p class="text-[10px] uppercase tracking-widest text-[#9CA3AF] mb-1">Evidencias del Servicio</p>
                            <div id="admin-evidencias-{{ $service['id'] }}" class="flex flex-wrap gap-2">
                                <!-- Se llenará con JS -->
                                <span class="text-xs text-[#374151] italic">Cargando...</span>
                            </div>
                        </div>

                        <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity mt-2">
                            <button 
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // En web.php, la ruta es /api-proxy/evidencias
        fetch('/api-proxy/evidencias')
            .then(res => res.json())
            .then(res => {
                if(res.success && res.data) {
                    renderAdminEvidencias(res.data);
                } else {
    ...
    function renderAdminEvidencias(evidencias) {
        ...
        // Reunimos los IDs de servicio mostrados
        const serviceIds = [];
        document.querySelectorAll('[id^="admin-evidencias-"]').forEach(el => {
            const id = el.id.replace('admin-evidencias-', '');
            serviceIds.push(id);
            el.innerHTML = ''; // limpiamos estado anterior (cargando)
        });

        serviceIds.forEach(serviceId => {
            const container = document.getElementById(`admin-evidencias-${serviceId}`);
            const serviceEvidencias = evidencias.filter(e => String(e.servicio_id) === String(serviceId));

            if(serviceEvidencias.length === 0) {
                container.innerHTML = '<span class="text-[10px] text-[#374151] italic">Sin evidencias</span>';
                return;
            }

            serviceEvidencias.forEach(ev => {
                const isPublic = ev.es_publica;
                const eyeIcon = isPublic ? 'fa-eye text-[#25B5DA]' : 'fa-eye-slash text-[#9CA3AF]';
                ...
                evNode.className = 'relative w-12 h-12 rounded overflow-hidden group/ev border border-[#374151] flex-shrink-0';
                evNode.innerHTML = `
                    <img src="${imgUrl}" class="w-full h-full object-cover ${isPublic ? 'opacity-100' : 'opacity-40'}">
                    <div class="absolute inset-0 bg-black/70 opacity-0 group-hover/ev:opacity-100 flex items-center justify-center gap-2 transition-opacity">
                        <button type="button" onclick="toggleEvidencia(${ev.id}, ${!isPublic})" class="text-[10px] hover:scale-125 transition-transform">
                            <i class="fas ${eyeIcon}"></i>
                        </button>
                        <button type="button" onclick="eliminarEvidencia(${ev.id})" class="text-[10px] text-red-500 hover:scale-125 transition-transform">
    ...
    async function toggleEvidencia(id, makePublic) {
        if(typeof showLoader === 'function') showLoader();
        try {
            ...
            if(data.success || res.ok) {
                const ev = adminEvidenciasData.find(e => e.id === id);
                if(ev) ev.es_publica = makePublic;
    ...
    async function eliminarEvidencia(id) {
        ...
        try {
            ...
            const data = await res.json();
            if(data.success || res.ok) {
                adminEvidenciasData = adminEvidenciasData.filter(e => e.id !== id);
                renderAdminEvidencias(adminEvidenciasData);
            } else {
